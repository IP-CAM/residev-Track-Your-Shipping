...
$('#awbsearch').on('click', function() {
  $.ajax({
    url: 'index.php?route=sale/order/awbpro&token=<?php echo $token; ?>',
    type: 'post',
    data: 'kurir=<?php echo $couriername; ?>&awb=<?php echo $awbnumber; ?>',
    dataType: 'json',
    beforeSend: function() {
      $('#awbsearch').button('loading');
    },
    complete: function() {
      $('#awbsearch').button('reset');
    },
    success: function(json) {
      ...
      if (json['error']) {
            ...
            if (json['error']['awb']) {
              $('#awbsearch').after('<div class="text-danger">' + json['error']['awb'] + '</div>');
            }
            if (json['error']['courier']) {
              $('#awbsearch').after('<div class="text-danger">' + json['error']['courier'] + '</div>');
            }
      }
      if (typeof (json['data']) != "undefined") {
        ...
        html += ' </tr>';
        if (typeof (json['data']['manifest']) != "undefined") {
